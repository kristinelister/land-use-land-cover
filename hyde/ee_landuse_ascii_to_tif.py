...
EE_COLLECTION = EE_FOLDER+ '/LandUse'
GS_FOLDER = 'HYDE'
...
AD_YEARS = np.arange(2016,2018)
URL = 'ftp://ftp.pbl.nl/hyde/hyde3.2/baseline/zip/{filename}'
DOWNLOAD_NAME = '{year}{AD}_lu.zip'
FOLDER_NAME = '{year}{AD}_lu'
FILE_NAME = '{varname}{year}{AD}.asc'
TEMP_FILE_NAME = 'temp_lu_{varname}{year}{AD}.tif'

PAUSE_FOR_OVERLOAD = True
NUM_ASSETS_AT_ONCE = 50

VARIABLES = ['cropland', 'grazing', 'pasture', 'rangeland', 'conv_rangeland','ir_rice', 'rf_rice', 'ir_norice', 'rf_norice', 'tot_irri', 'tot_rainfed','tot_rice']
...
def combine_tiffs(year,AD,NDV,varnames=VARIABLES,DATA_DIR=DATA_DIR):
    ...
    for i,name in enumerate(varnames):
        var_fnames[i] = DATA_DIR+'temp_lu_{name}{year}{AD}.tif'.format(name=name,year=year,AD=AD)
    
    #command to merge geotiffs
    cmd = ['gdal_merge.py','-separate','-a_nodata',str(NDV),'-o',DATA_DIR+'temp_lu_{year}{AD}.tif'.format(year=year,AD=AD)] + var_fnames
    subprocess.call(cmd)
...
def upload_asset(year,AD,NDV,DATA_DIR=DATA_DIR,EE_COLLECTION=EE_COLLECTION,GS_BUCKET=GS_BUCKET,VARIABLES=VARIABLES):
    ...
    TEMP_FILE_NAME = 'temp_lu_{year}{AD}.tif'
    ...
    #Get asset id 
    asset_id = EE_COLLECTION+'/'+'LandUse_'+str(year)+AD
...
for i,year in enumerate(YEARS):
    ...
    #Convert from ascii text to geotiff
